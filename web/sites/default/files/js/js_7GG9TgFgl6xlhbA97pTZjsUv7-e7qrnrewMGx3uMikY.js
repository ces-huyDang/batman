...
(function ($) {
  function getPostInformation() {
    ...
    for (let i = 0; i < $(".posts").length; i++) {
      ...
      const post = { id: "post" + i, top: element.getBoundingClientRect().top };
      postList.push(post);
    }
    return postList;
  }
  ...
  function createAnimation(postList) {
    ...
    postList.forEach((element) => {
      if (scrollPosition >= element.top) {
        // document.getElementById(element.id).addClass();
        $("#" + element.id).css("opacity", 1);
        $("#" + element.id).animate({ opacity: 1 }, 2000);
        // console.log(element.id + "id", element.top + "yes");
      } else {
        $("#" + element.id).css("opacity", 0.5);
        // $("#" + element.id).animate({ opacity: 0.5 }, 2000);
        // console.log(element.id + "id", element.top + "no");
      }
    });
  }
