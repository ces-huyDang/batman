...
(function ($) {
  function mouseEnter() {
    ...
    const btnColorClass = this.classList[1];
    ...
    for (let i = 0; i <= ratingScore; i++) {
      $("#" + i).removeClass(document.getElementById(i).classList[1]);
      $("#" + i).addClass(btnColor);
      console.log(document.getElementById(i).classList);
    }
  }

  function mouseOut() {
    $("#rating-score").find("#circle").remove();
    $("#rating-btn").find("#btn-group").remove();
    $("#rating-btn").append(newBtnGroup);
    console.log(newBtnGroup);
  }

  const btnGroup = $("#btn-group");
  const newBtnGroup = btnGroup.clone();

  $(document).ready(function () {
    const score = $(".rate-btn");
    score.mouseenter(mouseEnter);
    score.mouseout(mouseOut);
  });
})(jQuery);
;
/**
