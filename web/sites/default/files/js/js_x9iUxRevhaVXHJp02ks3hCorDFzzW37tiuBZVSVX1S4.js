...
(function ($) {
  ...
  function createAnimation(postList) {
    ...
    postList.forEach((element) => {
      ...
      if (scrollPosition >= element.top) {
        $("#post" + element.id).animate({ opacity: 1 }, 1000);
        typeWriterEffect($("#card-title" + element.id), text,  1);
      }
    });
  }
  ...
  $(document).ready(function () {
    $(window).scroll(function () {
      ...
      createAnimation(postList);

      $.each(myArray, function(item) {
        // Use a closure to encapsulate the state for each item
        (function(item) {
          // Check if the function has not been called for this item
          if (!item.hasOwnProperty('__functionCalled')) {
            // Call the function
            createAnimation(item);
      
            // Set a flag on the item to indicate that the function has been called
            item.__functionCalled = true;
          }
        })(item);
      });
    });
