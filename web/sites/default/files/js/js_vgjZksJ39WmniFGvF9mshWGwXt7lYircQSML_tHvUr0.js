...
(function ($) {
  function getPostInformation() {
    ...
    for (let i = 1; i < $(".posts").length; i++) {
      const element = document.getElementById("card-text" + i);
      const post = {
        id: i,
  ...
  function createAnimation(postList) {
    ...
    //   if (element.functionCalled === true || scrollPosition < element.top)
    //     return;
    ...
    for (let i = 0; i < postList.length; i++) {
      if (element.functionCalled === true) break;
      $("#post" + element.id).css("opacity", 0.3);
  ...
  $(window).scroll(function () {
    let postList = getPostInformation();
