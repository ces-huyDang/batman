...
(function ($) {
  ...
  function language() {
    ...
    const dropdownMenu = $('<ul class="dropdown-menu text-center"></ul>');
    const newParent = $("<div></div>", {
      html: '<button class="btn btn-sm btn-light dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false"></button>',
      id: originParent.attr("id"),
    });
    if (currentPath.startsWith("/vi", 0)) {
      dropdownMenu.html("English");
      newParent.html("Language");
    }
    originParent.replaceWith(newParent);
  ...
  const currentPath = window.location.location;
...
(function ($) {
  ...
  var nid = window.location.pathname.slice(6, window.location.pathname.length);
