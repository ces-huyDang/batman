  ...
  Drupal.behaviors.myDropdown = {
    attach: function (context, settings) {
      ...
      var newParent = $("<div></div>", {
        html: '<button class="btn btn-sm btn-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">Character</button>',
        id: originParent.attr("id"),
      });
      // newParent.find("*").addClass("text-visible text-center");
      originParent.replaceWith(newParent);
      var dropdownMenu = $(
        '<ul class="dropdown-menu bg-dark text-center"></ul>'
      );
      $.ajax({
        url: "/characters",
