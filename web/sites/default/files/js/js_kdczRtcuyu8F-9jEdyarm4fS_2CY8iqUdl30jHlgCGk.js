    ...
    attach: function (context, settings) {
      ...
      dropdownParent.hover(
        function () {
          ...
          $.ajax({
            url: "/characters", // Replace with the actual path
            success: function (data) {
              console.log(data);
              // Create and append the dropdown menu with dynamic data
              var dropdownMenu = $('<div class="dropdown"><ul class="dropdown-menu bg-dark"></ul></div>');
              data.forEach(function (item) {
                dropdownMenu.append(
                  '<li class="text-visible"><a href="' + item.link + '">' + item.term_name + "</a></li>"
                );
              });
